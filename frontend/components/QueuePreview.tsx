...
export default function QueuePreview({ className = '' }: QueuePreviewProps) {
  ...
  if (upcomingItems.length === 0) {
    return (
      <div className={`p-3 ${className}`}>
        <p className="text-xs text-secondary text-center">No upcoming tracks</p>
      </div>
    );
  }
  ...
  return (
    <div className={`p-2 ${className}`}>
      <p className="text-[10px] text-secondary uppercase tracking-wider mb-2 px-2">
      ...
      <ul className="space-y-1 max-h-80 overflow-y-auto">
        {upcomingItems.map((item, index) => (
          <li
            key={item.queueId}
            className="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-surface-elevated transition-colors"
          >
            <span className="text-xs text-tertiary w-5 text-center font-mono">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-secondary truncate">{item.song.artistName}</p>
            </div>
