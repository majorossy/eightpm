        ...
        return (
          <button
            ...
            className={`w-full px-4 py-3 text-left transition-colors ${
              !isAvailable ? 'opacity-35 cursor-not-allowed' : 'hover:bg-[#2a2520]'
            } ${
              option.value === preferredQuality && isAvailable ? 'bg-[#2a2520]' : ''
            } ${index !== qualityOptions.length - 1 ? 'border-b border-[#2a2520]' : ''}`}
          >
            <div className="flex items-start justify-between gap-2">
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className={`text-sm font-semibold ${isAvailable ? 'text-[#d4a060]' : 'text-[#6a6458]'}`}>{option.label}</span>
                  ...
                  {!isAvailable && (
                    <span className="text-[10px] text-[#6a6458] italic">Unavailable</span>
                  )}
                ...
                <div className={`text-xs ${isAvailable ? 'text-[#a89080]' : 'text-[#4a4438]'}`}>
