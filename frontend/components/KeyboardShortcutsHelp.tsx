    ...
    <Dialog.Root open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed inset-0 z-50 flex items-center justify-center p-4 pointer-events-none">
          <div className="bg-surface-card rounded-lg shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-y-auto pointer-events-auto">
            {/* Header */}
            <div className="sticky top-0 bg-surface-card border-b border-white/10 px-6 py-4 flex items-center justify-between">
              <Dialog.Title className="text-2xl font-bold text-white">Keyboard Shortcuts</Dialog.Title>
                        ...
                        <div className="flex items-center gap-2">
                          {shortcut.keys.map((key, keyIndex) => (
                            <span key={keyIndex} className="flex items-center gap-1">
                              ...
                              <kbd className="px-3 py-1.5 bg-surface-elevated text-gray-300 rounded-md border border-white/10 text-sm font-mono shadow-sm">
                                {key}
                              </kbd>
                            </span>
                          ))}
            ...
            {/* Footer */}
            <div className="sticky bottom-0 bg-surface-card border-t border-white/10 px-6 py-4">
              <p className="text-sm text-gray-400 text-center">
                Press <kbd className="px-2 py-1 bg-surface-elevated text-gray-300 rounded border border-white/10 text-xs font-mono">?</kbd> anytime to show this help
              </p>
            </div>
          </div>
        </Dialog.Content>
